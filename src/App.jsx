...
export default function App() {
    ...
    useEffect(() => {
        ...
        const unsubSessions = onSnapshot(collection(db, ...userPath, 'sessions'), (snapshot) => {
            ...
            // [수정] 훈련 기록을 '날짜순 내림차순(최신순)'으로 완벽하게 정렬합니다.
...
// --- [RecordView: 훈련 기록 전면 업데이트 (날짜 선택, 넥스트 숙제, 수정/삭제)] ---
        ...
        <div className="space-y-6 animate-in fade-in pb-10">
                    ...
                    <form onSubmit={handleSave} className="mt-5 space-y-4 animate-in slide-in-from-top-4">
                        <div className="space-y-3">
                            <div className="flex items-center justify-between bg-white p-3.5 rounded-xl border border-gray-200 shadow-sm min-w-0">
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest shrink-0 mr-2">운동 날짜</label>
                        ...
                        <textarea placeholder="오늘의 하이라이트 무브나 피드백을 자유롭게 기록하세요! (하루에 여러 개 작성 가능)" className="w-full min-w-0 bg-white p-4 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium h-24 shadow-sm placeholder:text-gray-400 leading-relaxed" value={newSummary} onChange={e => setNewSummary(e.target.value)} />
                        <div className="flex items-center gap-3 bg-white p-3.5 rounded-xl border border-orange-100 shadow-sm min-w-0">
            ...
            <div className="space-y-4">
                ...
                {sessions.map(s => {
                    ...
                    return isEditing ? (
                        // 수정 모드 UI
                        <div key={s.id} className="bg-blue-50/30 p-5 rounded-3xl border border-blue-200 shadow-sm space-y-3 animate-in fade-in">
                            <div className="flex justify-between items-center border-b border-blue-100 pb-2 mb-2">
                                <span className="text-xs font-bold text-blue-600 uppercase tracking-widest">기록 수정하기</span>
                            </div>
                            <div className="space-y-2">
                                <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-gray-200 shadow-sm min-w-0">
                                ...
                                <input type="text" value={editGymName} onChange={e => setEditGymName(e.target.value)} placeholder="암장명 입력" className="w-full min-w-0 bg-white p-3 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500 text-xs font-bold text-gray-700 shadow-sm" />
                                <select value={editTopLevel} onChange={e => setEditTopLevel(e.target.value)} className="w-full min-w-0 bg-white p-3 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500 text-xs font-bold text-gray-700 shadow-sm">
                                    {editGymLevels.map(lv => <option key={lv} value={lv}>{lv}</option>)}
                                </select>
                                <textarea placeholder="피드백 입력" className="w-full min-w-0 bg-white p-3 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500 text-xs font-medium h-20 shadow-sm leading-relaxed" value={editSummary} onChange={e => setEditSummary(e.target.value)} />
                                <div className="flex items-center gap-2 bg-white p-3 rounded-xl border border-orange-100 shadow-sm min-w-0">
                            ...
                            <div className="flex gap-2 justify-end mt-3">
                                ...
                                <button onClick={() => handleEditSave(s.id)} disabled={saving} className="px-4 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition-colors shadow-sm w-2/3">{saving ? '저장중...' : '수정 완료'}</button>
                            </div>
                        </div>
                    ) : (
                        // 일반 보기 모드 UI
                        <div key={s.id} className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-3 relative group">
                            ...
                            <span className={`text-xs font-bold px-3 py-1.5 rounded-lg shadow-sm border ${LEVEL_COLORS[s.topLevel] || 'bg-gray-100 text-gray-800'}`}>
                              {s.topLevel}
                                    ...
                                    <div className="flex flex-col gap-1 ml-1 opacity-50 hover:opacity-100 transition-opacity">
                                        <button onClick={() => handleEditClick(s)} className="p-1.5 bg-gray-50 rounded-lg border border-gray-100 text-gray-400 hover:text-blue-600 shadow-sm transition-colors"><Edit className="w-3 h-3" /></button>
                                        <button onClick={() => handleDelete(s.id)} className="p-1.5 bg-gray-50 rounded-lg border border-gray-100 text-gray-400 hover:text-rose-500 shadow-sm transition-colors"><Trash2 className="w-3 h-3" /></button>
                                    </div>
                                </div>
                            </div>
                            ...
                            {s.nextGoal && (
                                <div className="flex items-start gap-2 bg-orange-50 p-3 rounded-xl border border-orange-100/50 mt-2">
                                    ...
                                    <div>
                                        <span className="text-[9px] font-bold text-orange-600 uppercase tracking-widest block mb-0.5">Next Target</span>
